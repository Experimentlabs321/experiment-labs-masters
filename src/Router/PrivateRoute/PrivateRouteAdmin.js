...
const PrivateRouteAdmin = ({ children }) => {
  ...
  // Split the fullPath before the '?' to isolate the path part
  const [path, queryString] = fullPath.split("?");

  // Extract the ID from the path (the part after '/taskDetails/')
  const pathParts = path.split("/");
  const id = pathParts[2]; // This will give you '6639e0a8e7854998c458c4e3'

  // Extract the taskType from the query string
  ...
  // Function to show SweetAlert with logout option
