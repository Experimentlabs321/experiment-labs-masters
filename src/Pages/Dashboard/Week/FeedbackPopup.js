...
const FeedbackPopup = ({ taskData }) => {
   ...
   // console.log(taskData)
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_BACKEND_API}/getFeedback/${taskData?._id}`
            ...
            .then((response) => {
                ...
                if(response?.data){
                    const result = response?.data?.givenFeedbacks?.filter((data) => data?.submitterId === userInfo?._id)
                    setExisting(result)
                }
                else {
    ...
    useEffect(() => {
        axios
            ...
            .then((response) => {
                ...
                const feedbacks = response?.data?.courses?.find((data) => data?.courseId === courseId[0])
            ...
            >
                ...
                <DialogContent dividers>
                    <Typography gutterBottom>
                        ...
                        <div className="flex gap-3 items-center">
                            {
                                feedbacks?.categories?.map((category) => (<div>
                                    <p onClick={() => setSelectedCategory(category?.categoryName)} className={`border text-center rounded-3xl text-xl font-semibold p-2 ${selectedCategory === category?.categoryName ? 'bg-[#0040ff] text-[#fff] mt-6' : ''}`}>
                                    ...
                                    </p>
                                    {
                                        selectedCategory === category?.categoryName && (
                                            ...
                                            category?.rating === '5' && (
                                                <Rating
                                                    name="size-medium"
                                                    defaultValue={CategoryRatingValue[category?.categoryName]}
                                                    onChange={(e) =>
                                ...
                                selectedCategory === category?.categoryName && (
                                    category.feedbackItems?.map((item) => (
                                        <div className="lg:flex items-center justify-between lg:gap-20 mt-5">
                                            <p className="text-lg font-medium">{item?.feedbackItemName}</p>
                                            {
                                                selectedCategory === category?.categoryName && (
                ...
                Send feedback
                    ...
                    <div className="flex flex-col gap-3 justify-between w-full">
                        <input onChange={handleComment} className=" border border-black rounded-xl p-1" type="text" placeholder="comment" />
                        {
                            !checkZero && (
                                <input
                                    style={{
