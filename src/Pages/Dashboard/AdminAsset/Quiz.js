...
const Quiz = ({ allUsers }) => {
  ...
  const uniqueCourses = [
    ...new Set(quizs?.map((assignment) => assignment?.courseFullName)),
  ];
  const uniqueBatches = [
    ...new Set(
      quizs.flatMap((assignment) =>
        assignment.batches?.map((batch) => batch.batchName)
      )
    ),
  ];

  const filteredQuizs = quizs.filter((assignment) => {
  ...
  const sortedQuizs = filteredQuizs.sort((a, b) => {
          ...
          >
            <table className="min-w-full font-sans bg-white border border-gray-300">
              <thead className="bg-gray-800 text-white sticky top-0">
                <tr>
                  <th className="py-3 px-6 border-b text-left">Quiz name</th>
              ...
              <tbody>
                {paginatedClasses?.map((quiz, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 px-6 border-b text-left cursor-pointer">
                        {quiz?.taskName}
                      </td>
                      ...
                      <td className="py-4 px-6 border-b text-left">
                        {selectedBatch
                          ? selectedBatch
                          : quiz?.batches?.map((batch, index) => (
                              <span key={index}>
                                {batch.batchName}
                                {index < quiz.batches.length - 1 ? ", " : ""}
                              </span>
                            ))}
                      </td>
