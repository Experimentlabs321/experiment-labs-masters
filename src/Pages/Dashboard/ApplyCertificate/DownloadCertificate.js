...
const DownloadCertificate = () => {
  ...
  useEffect(() => {
    ...
    if (courseInfo?.batchId) {
      axios
        .get(
          `http://localhost:5000/api/v1/certificateTemplates/courseId/${courseId}/batchId/${courseInfo?.batchId}`
        )
        .then((response) => {
      ...
      axios
        .get(
          `http://localhost:5000/api/v1/batches/batchId/${courseInfo?.batchId}`
        )
        .then((response) => {
