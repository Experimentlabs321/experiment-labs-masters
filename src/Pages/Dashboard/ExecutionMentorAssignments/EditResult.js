...
const EditResult = (submittedAssignment) => {
  ...
  const [selectedEarningCategoryCategoryName, setSelectedEarningCategoryName] = useState('');
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_API}/tasks/assignments/?id=${taskid}`)
      .then((response) => {
  ...
  const handleSkillValue = (e, skill, mainValue) => {
    ...
    const categoryName = Assignment?.skillParameterData.find((item) => item?.categoryName === selectedCategoryName)
    const skillsName = categoryName?.skills.find((item) => item?.skillName === skill?.skillName)
  ...
  const handleParameterValue = (e, parameter, mainValue) => {
    setEdit(true)
    const categoryName = Assignment?.skillParameterData.find((item) => item?.categoryName === selectedCategoryName)
    const skillsName = categoryName.skills.find((item) => item?.skillName === selectedSkillName)
    const parameters = skillsName.parameters.find((item) => item?.parameterName === parameter?.parameterName)
  ...
  const handleEarningParameterValue = (e, earningItem, mainValue) => {
    setEdit(true)
    const categoryName = Assignment?.earningParameterData.find((item) => item?.categoryName === selectedEarningCategoryCategoryName)
    const earningItems = categoryName.earningItems.find((item) => item?.earningItemName === earningItem?.earningItemName)
                              ...
                              >
                                ...
                                <div className=" flex gap-2 ms-5">
                                  ...
                                  <input className="w-[50px] h-[25px] text-[14px] font-semibold border rounded-lg text-center"
                                    ...
                                    onChange={(e) => handleSkillValue(e, skill, CategoryMain[skill?.skillName])}
                                    placeholder='mark' />
                                          ...
                                          >
                                            ...
                                            <div className=" flex gap-2 ms-5">
                                              ...
                                              <input className="w-[50px] h-[25px] text-[14px] font-semibold border rounded-lg text-center"
                                                ...
                                                onChange={(e) => handleParameterValue(e, parameter, parameters[parameter.parameterName])}
                                                placeholder='mark' />
                                              {
                                                error1 && (
                                                  <>
                                              ...
                                              <span>/{parameters[parameter.parameterName]}</span>
                              ...
                              >
                                ...
                                <div className=" flex gap-2 ms-5">
                                  ...
                                  <input className="w-[50px] h-[25px] text-[14px] font-semibold border rounded-lg text-center"
                                    ...
                                    onChange={(e) => handleEarningParameterValue(e, earningItem, earning[earningItem.earningItemName])}
                                    placeholder='mark' />
                                  {
                                    error3 && (
                                      <span className="text-[red] text-[10px]">error</span>
                                  ...
                                  <span>/{earning[earningItem.earningItemName]}</span>
