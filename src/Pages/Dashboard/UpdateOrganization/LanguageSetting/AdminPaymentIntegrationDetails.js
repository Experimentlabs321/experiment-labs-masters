...
const AdminPaymentIntegrationDetails = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const item = await axios.post(
      `${process.env.REACT_APP_SERVER_API}/api/v1/language/addUpdateOrganizationSubDetails/paymentIntegration/organizationId/${userInfo?.organizationId}`,
      itemDetail
    );
    console.log(item)
          ...
          <form onSubmit={handleSubmit} className='mt-2 border p-4 rounded-xl '>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div className='flex justify-between items-center w-[100%]'>
                ...
                <input name='keyId' defaultValue={itemDetails?.keyId} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
              </div>
              <div className='flex justify-between items-center w-[100%]'>
                <p className='text-lg font-medium'>Key Secret</p>
                <input name='keySecret' defaultValue={itemDetails?.keySecret} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
              </div>
              <div className='flex justify-between items-center w-[100%]'>
