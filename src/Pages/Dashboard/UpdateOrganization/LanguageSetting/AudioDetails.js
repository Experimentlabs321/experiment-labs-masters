...
const AudioDetails = () => {
    ...
    useEffect(() => {
        if (userInfo) {
            ...
            axios
                .get(
                    `${process.env.REACT_APP_SERVERLESS_API}/api/v1/language/getContentManageSubDetailsByOrganizationAndName/audio/organizationsId/${userInfo?.organizationId}`
                )
                .then((response) => {
    ...
    const handleSubmit = async (event) => {
        ...
        const newManageAudioInName = form.manageAudioIn?.value;
        const newPreviewModeName = form.previewMode?.value;
        const newAudioTopicNameName = form.audioTopicName?.value;
        const newUploadAudioName = form.uploadAudio?.value;
        ...
        const itemDetail = {
            manageAudioIn: newManageAudioInName,
            previewMode: newPreviewModeName,
            audioTopicName: newAudioTopicNameName,
            uploadAudio: newUploadAudioName,
        ...
        const item = await axios.post(
            `${process.env.REACT_APP_SERVERLESS_API}/api/v1/language/addContentManageSubDetails/audio/organizationId/${userInfo?.organizationId}`,
            itemDetail
        );
        console.log(item)
                    ...
                    <form onSubmit={handleSubmit} className='mt-2 border p-4 rounded-xl'>
                        <div className=' grid grid-cols-2 gap-4'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Manage Audio in</p>
                                <input name='manageAudioIn' defaultValue={itemDetails?.manageAudioIn} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Audio Topic Name</p>
                                <input name='audioTopicName' defaultValue={itemDetails?.audioTopicName} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Upload Audio</p>
                                <input name='uploadAudio' defaultValue={itemDetails?.uploadAudio} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
