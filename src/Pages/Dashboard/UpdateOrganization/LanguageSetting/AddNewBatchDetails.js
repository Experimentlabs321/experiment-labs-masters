...
const AddNewBatchDetails = () => {
    ...
    useEffect(() => {
        if (userInfo) {
            ...
            axios
                .get(
                    `${process.env.REACT_APP_SERVER_API}/api/v1/language/getContentManageSubDetailsByOrganizationAndName/addNewBatch/organizationsId/${userInfo?.organizationId}`
                )
                .then((response) => {
    ...
    const handleSubmit = async (event) => {
        ...
        const item = await axios.post(
            `${process.env.REACT_APP_SERVER_API}/api/v1/language/addContentManageSubDetails/addNewBatch/organizationId/${userInfo?.organizationId}`,
            itemDetail
        );
        console.log(item)
                    ...
                    <form onSubmit={handleSubmit} className='mt-2 border p-4 rounded-xl'>
                        <div className=' grid grid-cols-2 gap-4'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                ...
                                <input name='addNewBatch' defaultValue={itemDetails?.addNewBatch} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Edit Batch</p>
                                <input name='editBatch' defaultValue={itemDetails?.editBatch} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Batch Name</p>
                                <input name='batchName' defaultValue={itemDetails?.batchName} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Batch Starting Date</p>
                                <input name='batchStartingDate' defaultValue={itemDetails?.batchStartingDate} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>

                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Batch Ending Date</p>
                                <input name='batchEndingDate' defaultValue={itemDetails?.batchEndingDate} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Batch Price</p>
                                <input name='batchPrice' defaultValue={itemDetails?.batchPrice} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
