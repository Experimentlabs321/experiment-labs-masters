...
const VideoDetails = () => {
    ...
    useEffect(() => {
        if (userInfo) {
            ...
            axios
                .get(
                    `${process.env.REACT_APP_SERVERLESS_API}/api/v1/language/getContentManageSubDetailsByOrganizationAndName/video/organizationsId/${userInfo?.organizationId}`
                )
                .then((response) => {
    ...
    const handleSubmit = async (event) => {
        ...
        const newManageVideoInName = form.manageVideoIn?.value;
        const newPreviewModeName = form.previewMode?.value;
        const newVideoTopicNameName = form.videoTopicName?.value;
        const newUploadVideoName = form.uploadVideo?.value;
        const newDragAndDropName = form.dragAndDrop?.value;
        const newOrName = form.or?.value;
        const newBrowserName = form.browser?.value;
        const newYoutubeName = form.youtube?.value;
        ...
        const itemDetail = {
            manageVideoIn: newManageVideoInName,
            previewMode: newPreviewModeName,
            videoTopicName: newVideoTopicNameName,
            uploadVideo: newUploadVideoName,
            dragAndDrop: newDragAndDropName,
            or: newOrName,
            browser: newBrowserName,
            youtube: newYoutubeName,
        ...
        const item = await axios.post(
            `${process.env.REACT_APP_SERVERLESS_API}/api/v1/language/addContentManageSubDetails/video/organizationId/${userInfo?.organizationId}`,
            itemDetail
        );
        console.log(item)
                    ...
                    <form onSubmit={handleSubmit} className='mt-2 border p-4 rounded-xl'>
                        <div className=' grid grid-cols-2 gap-4'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Manage Video in</p>
                                <input name='manageVideoIn' defaultValue={itemDetails?.manageVideoIn} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Video Topic Name</p>
                                <input name='videoTopicName' defaultValue={itemDetails?.videoTopicName} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                                <p className='text-lg font-medium'>Upload Video</p>
                                <input name='uploadVideo' defaultValue={itemDetails?.uploadVideo} type='text' className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
                            ...
                            <div className='flex justify-between items-center w-[100%]'>
                                ...
                                <input name='youtube' type='text' defaultValue={itemDetails?.youtube} className='border border-slate-300 rounded-lg p-2 w-[300px]' placeholder='Text here' />
                            </div>
                            <div className='flex justify-between items-center w-[100%]'>
