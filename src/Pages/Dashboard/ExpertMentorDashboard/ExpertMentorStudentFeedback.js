...
const ExpertMentorStudentFeedback = () => {
    ...
    useEffect(() => {
        axios
            ...
            .then((response) => {
                ...
                const feedbacks = response?.data?.courses?.find((data) => data?.courseId === selectedCourse?._id)
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_BACKEND_API}/getFeedbacks/${selectedCourse?._id}`)
            .then((response) => {
                const filteredFeedbacks = response?.data.reduce((acc, accessClass) => {
                    const filteredAccessClass = {
                        ...accessClass,
                        givenFeedbacks: accessClass?.givenFeedbacks.reduce((feedbackAcc, givenFeedback) => {
                            const filteredGivenFeedback = {
                                ...givenFeedback,
                                categories: givenFeedback?.categories.reduce((categoryAcc, category) => {
                                    const filteredCategory = {
                                        ...category,
                                        feedbackItems: category?.feedbackItems.reduce((feedbackItemAcc, feedbackItem) => {
                                            const filteredFeedbackItem = {
                                                ...feedbackItem,
                                                giveAccess: feedbackItem?.giveAccess?.filter(access => access === "Expert mentor") || []
                                            };
            ...
            <Layout>
...

                {
                    (selectedTab1 === 'All' && classes?.length) ? (
                                            ...
                                            selectedClassShow?.categories?.map((category) => (
                                                category?.feedbackItems?.map((item) => (
                                                    <div className="flex flex-col items-center h-[100%] justify-end">
                                                        <div className="">
                                                            <div><img className="h-[150px]" src={item?.selectedIcon} alt='Icon' /></div>
                                                        </div>
                                                        <p className="text-lg font-bold">{item?.feedbackItemName}</p>
                                                        <div className="my-3">
                                                            {
                                                                givenRating.map((data) => (
                                                                    (data.feedbackItemName == item?.feedbackItemName) && (

                                                                        <>
                                                                            {
                                                                                (mainCourse[item?.feedbackItemName] === '10') && (
                                                                            ...
                                                                            {
                                                                                (mainCourse[item?.feedbackItemName] === '5') && (
                                                        ...
                                                        {
                                                            classes?.map((data) => (
                                                                (data?.taskId === selectedClass?.taskId || data?.taskId === selectedClass) && (
                                                                    <div>
                                                                        <p className="text-lg font-normal">{data?.givenFeedbacks.length}  students</p>
                                                                        {
                                                                            data?.givenFeedbacks?.map((givenFeedback) => (
                                                                                ...
                                                                                givenFeedback.categories?.map((category) => (
                                                                                    category.feedbackItems?.map((m,) => (
                                                                                        ...
                                                                                        (m.feedbackItemName === item?.feedbackItemName) && (
                                                                                            <>
                                                                                                <p onClick={() => handleSelectUser(givenFeedback?.submitterName, givenFeedback?.comment)} className="mt-3 text-lg font-bold cursor-pointer">{givenFeedback?.submitterName}</p>
                                                                                                ...
                                                                                                <div>
                                                                                                    ...
                                                                                                    >
                                                                                                        <DialogTitle sx={{ m: 0, px: 20 }} id="customized-dialog-title">
                                                                                                ...
                                                                                                {
                                                                                                    mainCourse[m?.feedbackItemName] === '5' && (
                                                                                                        <Rating
                                                                                                            readOnly
                                                                                                            name="read-only"
                                                                                                ...
                                                                                                {
                                                                                                    mainCourse[m?.feedbackItemName] === '10' && (
                                                                                                        <Rating
                                                                                                            readOnly
                                                                                                            // name="size-medium"
                                                                                                            name="read-only"
                                    ...
                                    classes?.map((data) => (
                                        (data?.taskId === selectedClass?.taskId || data?.taskId === selectedClass) && (
                                            ...
                                            data?.givenFeedbacks?.map((givenFeedback) => (
                                                ...
                                                >
                                                    <p className=" text-base font-semibold">{givenFeedback?.comment} </p>
                                                    <p className="text-[#838383] text-base font-normal my-2">{userInfo?.organizationName}</p>
                                                    <p className="flex justify-between text-[#838383] text-base font-normal mt-5   "> {givenFeedback?.submitterName} <span> {givenFeedback?.dateAndTime}</span></p>
                                                </div>
                                                /*    givenFeedback.categories?.map((category) => (
                                                       category.feedbackItems?.map((m,) => (
                        ...
                        :
                        (
                            <p className="text-[red] ms-10 mt-10 text-lg font-semibold text-center">There is no Result</p>
                        )
                }
