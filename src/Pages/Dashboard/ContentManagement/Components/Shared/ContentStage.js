...
const ContentStage = ({ contentStage, setContentStage }) => {
    ...
    const toggleTag = (tag) => {
        if (contentStage?.includes(tag)) {
            setContentStage(contentStage?.filter((selectedTag) => selectedTag !== tag));
        } else {
        ...
        <div className="">
            ...
            <div className="flex flex-row gap-5 flex-wrap items-center">
                {tagsData?.map((tag) => (
                    <div
                        key={tag}
                        className={`flex items-center justify-between py-2 px-4 rounded-full cursor-pointer ${contentStage?.includes(tag)
                            ? 'bg-green text-white'
